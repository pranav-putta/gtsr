...
def display_interval_plot(t: np.ndarray, v: np.ndarray, accel: np.ndarray, brake: np.ndarray,
...
def display_throttle(time: np.ndarray, velocity: np.ndarray, throttle: np.ndarray):
    """
    ...
    plt.xlabel('Time (seconds)')
    plt.ylabel('Velocity (m/s)')
    plt.title('Raw Peak Detection')

    plt.plot(time, velocity, 'b', label='Derived velocity')
    plt.plot(time, throttle, 'r', label='Throttle')
    plt.legend()
    plt.show()
